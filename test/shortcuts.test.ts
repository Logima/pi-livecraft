...
test('conserve exactement les touches d’un raccourci', () => {
  ...
  assert.equal(defaultShortcuts['open-thinking'], undefined)
  assert.equal(defaultShortcuts['open-model'], undefined)
  assert.equal(defaultShortcuts['open-agent'], undefined)
