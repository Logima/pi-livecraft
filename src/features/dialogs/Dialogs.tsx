...
export function AskUserQuestionDialog(
  ...
  function isAnswered(index: number): boolean {
    return selectedOptions[index].length > 0
      || (!request.questions[index].multiSelect && freeText[index].trim().length > 0)
  }
  ...
  function toggle(questionIndex: number, option: string): void {
    ...
    if (!question.multiSelect && !wasSelected && questionIndex < request.questions.length - 1)
  ...
  async function respond(cancelled: boolean): Promise<void> {
    try {
      ...
      await sendPiCommand(dialog.sessionId, {
    ...
    >
      ...
      >
        ...
        <nav aria-label='Questionnaire questions' className='ask-user-question-tabs'>
          {request.questions.map((item, index) => (
            <button
              ...
              className={index === activeQuestion ? 'active' : isAnswered(index) ? 'answered' : ''}
          ...
          <fieldset>
            ...
            <p className='ask-user-question-hint'>
              {question.multiSelect
                ...
                : 'Choose an answer or write your own'}
            </p>
              ...
              {question.options.map((option) => {
                ...
                return (
                  <button
                    ...
                    key={option.label}
                    onClick={() => toggle(activeQuestion, option.label)}
                    type='button'
                  >
                    <span aria-hidden='true' className='ask-user-option-mark'>
                      {selected ? '✓' : ''}
                    </span>
                    <span>
                ...
                onChange={(event) =>
                  setFreeText((current) =>
                    current.map((text, index) =>
                      index === activeQuestion ? event.target.value : text
                    )
                  )}
                placeholder='Or type your own answer…'
                rows={2}
                value={freeText[activeQuestion]}
              />
            )}
          </fieldset>
        </div>
        <div className='ask-user-question-actions'>
          ...
          <div>
            {activeQuestion > 0 && (
              <button onClick={() => setActiveQuestion((index) => index - 1)} type='button'>
                Previous
              </button>
            )}
            {lastQuestion
              ? (
                <button disabled={!complete} onClick={() => void respond(false)} type='button'>
                  Submit answers
                </button>
              )
              : (
                <button
                  disabled={!isAnswered(activeQuestion)}
        ...
        <div className='modal-actions'>
          {request.method === 'confirm' && (
            <>
              ...
              <button onClick={() => void respond({ confirmed: false })} type='button'>
                Decline
              </button>
            </>
          )}
