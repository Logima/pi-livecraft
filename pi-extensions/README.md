...
- `subagents.ts` publishes validated pi-subagents lifecycle snapshots through the reserved `pi-livecraft.subagents` status key; it never writes session messages or exposes filesystem paths.
